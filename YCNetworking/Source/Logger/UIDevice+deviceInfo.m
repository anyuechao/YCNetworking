...
static NSString *YCKeychainServiceName = @"YCKeychainServiceName";
static NSString *YCUDIDName = @"YCNetworkingUDID";
static NSString *YCPasteboardType = @"YCPasteboardType";
...
- (NSString *)yc_uuid {
    NSString *key = @"YCNetworkingUUID";
    NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:key];
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self yc_createUUID] forKey:key];
...
- (NSString *) yc_macaddressMD5 {
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] yc_macaddress];
...
#pragma mark - UDID相关
- (NSString *)getUDID {
    NSData *udidData = [self searchKeychainCopyMatching:YCUDIDName];
    NSString *udid = nil;
    if (udidData != nil) {
    ...
    if (udid.length == 0) {
        udid = [self readPasteBoradforIdentifier:YCUDIDName];
    }
    return udid;
}
...
- (void)saveUDID:(NSString *)udid {
    ...
    NSData *udidData = [self searchKeychainCopyMatching:YCUDIDName];
    if (udidData == nil) {
        saveOk = [self createKeychainValue:udid forIdentifier:YCUDIDName];
    } else {
        saveOk = [self updateKeychainValue:udid forIdentifier:YCUDIDName];
    }
    if (!saveOk) {
        [self createPasteBoradValue:udid forIdentifier:YCUDIDName];
    }
}
...
- (NSMutableDictionary *)newSearchDictionary:(NSString *)identifier {
    ...
    [searchDictionary setObject:YCKeychainServiceName forKey:(__bridge id)kSecAttrService];
...
- (void)createPasteBoradValue:(NSString *)value forIdentifier:(NSString *)identifier {
    UIPasteboard *pb = [UIPasteboard pasteboardWithName:YCKeychainServiceName create:YES];
    NSDictionary *dict = [NSDictionary dictionaryWithObject:value forKey:identifier];
    NSData *dictData = [NSKeyedArchiver archivedDataWithRootObject:dict];
    [pb setData:dictData forPasteboardType:YCPasteboardType];
}

- (NSString *)readPasteBoradforIdentifier:(NSString *)identifier {
    UIPasteboard *pb = [UIPasteboard pasteboardWithName:YCKeychainServiceName create:YES];
    NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithData:[pb dataForPasteboardType:YCPasteboardType]];
